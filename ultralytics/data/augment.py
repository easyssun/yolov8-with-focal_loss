...
class RandomHSV:
    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            hgain, sgain, vgain = np.random.uniform(-.5, .5, 3) * [self.hgain, self.sgain, self.vgain] + .5 # random gains
            hue, sat, val = cv2.split(cv2.cvtColor(img, cv2.COLOR_BGR2Luv))

            lut_hue = np.array([round((x * hgain) / (x * hgain + (255 - x) * (1 - hgain)) * 255) for x in range(256)], dtype=np.uint8)
            lut_sat = np.array([round((x * sgain) / (x * sgain + (255 - x) * (1 - sgain)) * 255) for x in range(256)], dtype=np.uint8)
            lut_val = np.array([round((x * vgain) / (x * vgain + (255 - x) * (1 - vgain)) * 255) for x in range(256)], dtype=np.uint8)

            im_hsv = cv2.merge((cv2.LUT(hue, lut_hue), cv2.LUT(sat, lut_sat), cv2.LUT(val, lut_val)))
            cv2.cvtColor(im_hsv, cv2.COLOR_Luv2BGR, dst=img)  # no return needed
        return labels
